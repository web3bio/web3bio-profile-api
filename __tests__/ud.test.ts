import { GET } from "@/app/api/profile/unstoppabledomains/[handle]/route";
...
describe("Test For Unstoppable Domains Profile API", () => {
  it("It should response 200 for nykma.blockchain", async () => {
    const res = await GET(generateRequestBody("nykma.blockchain"));
  ...
  it("It should response 200 for sandy.x", async () => {
    const res = await GET(generateRequestBody("sandy.nft"));
    expect(res.status).toBe(200);
    const json = await res.json();
  ...
  it("It should response 200 for 2024.hi", async () => {
    const res = await GET(generateRequestBody("2024.hi"));
    expect(res.status).toBe(200);
  });
});
