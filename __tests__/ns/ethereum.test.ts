import { GET } from "@/app/api/ns/ethereum/[handle]/route";
import { GET as GETENS } from "@/app/api/ns/ens/[handle]/route";
...
describe("Test For Ethereum Profile API", () => {
  ...
  it("It should response 200 for planetable.eth", async () => {
    const res = await GET(generateRequestBody("planetable.eth"));
    const json = await res.json();
