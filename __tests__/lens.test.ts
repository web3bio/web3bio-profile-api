import { GET } from "@/app/api/profile/lens/[handle]/route";
...
describe("Test For Lens Profile API", () => {
  ...
  it("It should response 200 for 0x934b510d4c9103e6a87aef13b816fb080286d649", async () => {
    const res = await GET(generateRequestBody('0x934b510d4c9103e6a87aef13b816fb080286d649'))
    expect(res.status).toBe(200);
    const json = await res.json();
  ...
  it("It should response 404 for 0xc0074d4F69F4281d7a8EB4D266348BA9F7599E0A", async () => {
    const res = await GET(generateRequestBody('0xc0074d4F69F4281d7a8EB4D266348BA9F7599E0A'))
    expect(res.status).toBe(404);
